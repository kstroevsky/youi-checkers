...
describe('AI model plumbing', () => {
  ...
  it('encodes the model input planes with the expected fixed shape', () => {
    const state = createInitialState(
      withConfig({ drawRule: 'threefold', scoringMode: 'off' }),
    );
    const encoded = encodeStateForModel(state);

    expect(encoded).toHaveLength(16 * 36);
    expect(
      encoded.slice(13 * 36, 14 * 36).reduce((sum, value) => sum + value, 0),
    ).toBe(18);
    expect(
      encoded.slice(14 * 36, 15 * 36).reduce((sum, value) => sum + value, 0),
    ).toBe(6);
  });
  ...
  it('treats an HTML app-shell model response as a missing model without importing ONNX runtime', async () => {
    ...
    const guidance = await getModelGuidance(
      state,
      withConfig({ drawRule: 'threefold', scoringMode: 'off' }),
    );
  ...
  it('treats HTML-looking model bytes as missing even when the content type is non-html', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
  ...
  it('accepts complete model bytes and creates an ONNX guidance session from them', async () => {
    const modelBytes = new Uint8Array([0x08, 0x01, 0x12, 0x04]);
    const fetchMock = vi.fn().mockResolvedValue(
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(createSessionSpy).toHaveBeenCalledWith(expect.any(Uint8Array), {
      executionProviders: ['wasm'],
      graphOptimizationLevel: 'all',
    });
    expect(createSessionSpy.mock.calls[0]?.[0]).toEqual(modelBytes);
  ...
  it('rejects partial model responses so a cached range cannot seed a session', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(new Uint8Array([0x08, 0x01]), {
        headers: {
          'content-type': 'application/octet-stream',
        },
        status: 206,
