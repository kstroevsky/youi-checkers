...
At runtime the worker fetches the exported artifact as complete bytes, rejects non-model/HTML fallback responses, and only then lazy-loads the optional ONNX WASM runtime. This keeps a missing deployment artifact from pulling inference code into a search-only session and prevents a partial range response from being mistaken for a model file.

### Deployment-performance contract

The training output participates in runtime performance only after it has
passed the artifact boundary. [`src/ai/model/guidance.ts`](../src/ai/model/guidance.ts)
uses three worker-local promises—model bytes, ONNX module, and inference
session—to coalesce concurrent requests and initialize each resource at most
once per worker lifetime.

```mermaid
sequenceDiagram
  participant Search as AI worker request
  participant Bytes as modelBytesPromise
  participant Ort as ortModulePromise
  participant Session as sessionPromise

  Search->>Bytes: GET /models/ai-policy-value.onnx
  Bytes-->>Search: complete valid bytes or null
  alt bytes are valid
    Search->>Ort: dynamic import onnxruntime-web/wasm
    Ort-->>Search: WASM runtime
    Search->>Session: create optimized WASM session
    Session-->>Search: shared session
  else missing or unusable artifact
    Search-->>Search: heuristic/search-only guidance
  end
```

This ordering is important. Checking artifact availability before importing the
optional runtime removes ONNX initialization from model-free sessions. Promise
memoization prevents multiple simultaneous computer-turn requests from
duplicating fetch, module evaluation, or session construction. A failed load is
also memoized for that worker lifetime; deploying a previously missing model
therefore requires a worker/page restart before that live worker retries.

The optimization does not alter the trained graph, policy targets, value
targets, or the behavior of a successfully loaded model. It changes only when
optional inference infrastructure becomes resident. Once a valid model is
loaded, inference still encodes the full `16 x 6 x 6` tensor and masks policy
logits against the canonical domain legal-action list.
